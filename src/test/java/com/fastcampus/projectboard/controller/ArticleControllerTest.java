...
@WebMvcTest(ArticleController.class) // WebMvcTest는 골라서 test 가능 -> 가벼워짐
class ArticleControllerTest {
    ...
    public void givenNothing_whenRequestingArticlesView_thenReturnsArticlesView() throws Exception {
        // Given
        given(articleService.searchArticles(eq(null), eq(null), any(Pageable.class))).willReturn(Page.empty()); // 검색어 X
    ...
    public void givenSearchKeyword_whenSearchingArticlesView_thenReturnsArticlesView() throws Exception {
        ...
        given(articleService.searchArticles(eq(searchType), eq(searchValue), any(Pageable.class))).willReturn(Page.empty()); // 검색어 O
        given(paginationService.getPaginationBarNumbers(anyInt(), anyInt())).willReturn(List.of(0, 1, 2, 3, 4));
    ...
    @WithMockUser // 사용자 인증 정보 Mocking해서 넣어주는 어노테이션
    @DisplayName("[view][GET] 게시글 페이지 - 정상 호출, 인증된 사용자") // `/articles{article-id}` - 게시글 페이지
    @Test
    public void givenNothing_whenRequestingArticleView_thenReturnsArticleView() throws Exception {
        // Given
        Long articleId = 1L;
        long totalCount = 1L; // 게시글 총 개수
        given(articleService.getArticleWithComments(articleId)).willReturn(createArticleWithCommentsDto()); // articleId에 대한 가짜 게시글 데이터 반환(생성)
        given(articleService.getArticleCount()).willReturn(totalCount); // 게시글 총 개수 반환
    ...
    @DisplayName("[view][GET] 게시글 해시태그 검색 페이지 - 정상 호출") // `/articles/search-hashtag` - 게시판 해시태그 검색 페이지
    @Test
    public void givenNothing_whenRequestingArticleSearchHashtagView_thenReturnsArticleSearchHashtagView() throws Exception {
        ...
        given(articleService.searchArticlesViaHashtag(eq(null), any(Pageable.class))).willReturn(Page.empty()); // 입력 X
    ...
    @DisplayName("[view][GET] 게시글 해시태그 검색 페이지 - 정상 호출, 해시태그 입력") // `/articles/search-hashtag` - 게시판 해시태그 검색 페이지
    @Test
    public void givenHashtag_whenRequestingArticleSearchHashtagView_thenReturnsArticleSearchHashtagView() throws Exception {
        // Given
        String hashtag = "#java";
        List<String> hashtags = List.of("#java", "#spring", "#boot");
        given(articleService.searchArticlesViaHashtag(eq(hashtag), any(Pageable.class))).willReturn(Page.empty()); // 입력 O
        given(articleService.getHashtags()).willReturn(hashtags);
        given(paginationService.getPaginationBarNumbers(anyInt(), anyInt())).willReturn(List.of(1, 2, 3, 4, 5));
    ...
    @WithUserDetails(value = "unoTest", setupBefore = TestExecutionEvent.TEST_EXECUTION) // 실제 사용자 정보를 넣어주는 어노테이션
